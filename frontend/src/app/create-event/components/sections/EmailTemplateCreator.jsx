...
import styles from "./EmailTemplateCreator.module.css";
...
const EmailTemplateCreator = ({
  ...
  useEffect(() => {
    if (!eventData.emailTemplates || eventData.emailTemplates.length === 0) {
      const defaultTemplate = {
        ...
        subject_line: `Invitation: ${eventData.title || "Your Event"}`,
      ...
      updateEventData({
        emailTemplates: [defaultTemplate],
      });
    }
  }, [eventData.emailTemplates, eventData.title, updateEventData]);

  const emailTemplates = eventData.emailTemplates || [];
  ...
  const addNewTemplate = () => {
    ...
    const newTemplate = {
      ...
      sender_email: "sender@event-sync.com",
      reply_to: "reply@event-sync.com",
      subject_line: `${availableCategory.charAt(0).toUpperCase() + availableCategory.slice(1)}: ${eventData.title || "Your Event"}`,
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      ...
      if (!hasActiveTemplate && emailTemplates.length > 0) {
        const updatedTemplates = [...emailTemplates];
        updatedTemplates[0].status = "active";
        updateEventData({
          emailTemplates: updatedTemplates,
        });
      }

      toast.success("Email templates configured successfully!", {
        position: "top-center",
        autoClose: 2000,
      });
      onNext();
  ...
  const getPreviewContent = () => {
    ...
    const eventName = eventData?.title || "Your Event";
    const eventDate = eventData?.subEvents?.[0]?.date
      ? new Date(eventData.subEvents[0].date).toLocaleDateString("en-US", {
          year: "numeric",
          month: "long",
          day: "numeric",
        })
    ...
    const rsvpLink = `${process.env.NEXT_PUBLIC_HOST || "http://localhost:3000"}/${eventData.public_id}/rsvp?guestId=sample-guest-id`;

    return {
      eventName,
      eventDate,
      eventDescription,
      logoLink: eventData.logo_url || "",
        ...
        <div className={styles.emailHeader}>
          <div className={styles.emailMeta}>
            ...
            {currentTemplate.subject_line ||
              `Invitation: ${eventData?.title || "Your Event"}`}
          </div>
          <div className={styles.emailMeta}>
            <strong>From:</strong>{" "}
  ...
  return (
    <form
      className={styles.formSection + " " + styles.emailTemplateCreator}
      onSubmit={handleSubmit}
    >
      <div className={styles.sectionHeader}>
        <div className={styles.sectionIcon}>📧</div>
        <div>
          <h2 className={styles.sectionTitle}>Email Template Creator</h2>
          <p className={styles.sectionDescription}>
            Create and customize email templates for invitations, reminders, and
            updates.
          </p>
        </div>
      </div>
            ...
            <div className={styles.controlSection}>
              ...
              <div className={styles.formGroup}>
                ...
                <input
                  ...
                  placeholder={`Invitation: ${eventData?.title || "Your Event"}`}
                />
                {errors.subject_line && (
