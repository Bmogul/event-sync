...
const GuestListSection = ({
  ...
  const getSubEventGuestCount = (subEventId) => {
    return eventData.guests?.filter(guest =>
  ...
  const handleAddMemberToGroup = () => {
    ...
    if (editingMemberIndex >= 0) {
      ...
      toast.success('Member updated!', { position: 'top-center', autoClose: 1500 })
    } else {
      setTempGroupMembers([...tempGroupMembers, memberData])
      toast.success('Member added to group!', { position: 'top-center', autoClose: 1500 })
    }
  ...
  const handleRemoveMember = (index) => {
    ...
    toast.success('Member removed from group', { position: 'top-center', autoClose: 1500 })
  }
  ...
  const handleCreateGroup = () => {
    ...
    if (tempGroupMembers.length === 0) {
      toast.error('Please add at least one member to the group', { position: 'top-center' })
      return
    }

    const pointOfContacts = tempGroupMembers.filter(member => member.isPointOfContact)
    if (pointOfContacts.length === 0) {
      toast.error('Please select at least one member as point of contact', { position: 'top-center' })
      return
    }

    const groupId = Date.now()
    const groupColor = groupColors[((eventData.guestGroups?.length || 0)) % groupColors.length]
    ...
    toast.success(`Group "${newGroup.name}" created with ${tempGroupMembers.length} members!`, {
      position: 'top-center',
      autoClose: 3000
    })
  }
  ...
  const handleAddIndividualGuest = () => {
    ...
    const guest = {
      id: guestId,
      order: (eventData.guests?.filter(g => g.group === groupTitle).length || 0) + 1,
      name: newGuest.name,
      email: newGuest.email,
      phone: newGuest.phone || '',
      gender: newGuest.gender || '',
    ...
    setShowGuestForm(false)
    toast.success('Individual guest added successfully!', { position: 'top-center', autoClose: 2000 })
  }
  ...
  const handleRemoveGuest = (guestId) => {
    ...
    if (group) {
      ...
      if (group.size <= 0) {
        ...
        const remainingGroupMembers = updatedGuests.filter(g => g.group === group.title)
    ...
    toast.success('Guest removed successfully', { position: 'top-center', autoClose: 2000 })
  }
  ...
  const handleFileUpload = (event) => {
    ...
    toast.info(`Processing file: ${file.name}...`, { position: 'top-center', autoClose: 3000 })

    setTimeout(() => {
      toast.success('File upload functionality coming soon!', { position: 'top-center' })
    }, 2000)
  }
    ...
    <div className={styles.formSection}>
      <div className={styles.sectionHeader}>
        ...
        <div>
          ...
          <p className={styles.sectionDescription}>
            Add your guests and organize them into groups. Groups are automatically created based on your selection.
          </p>
        </div>
      </div>
      ...
      <div className={styles.statsContainer}>
        ...
        {subEvents.map(subEvent => (
          <div key={subEvent.id} className={styles.statCard}>
            <div className={styles.statNumber}>{getSubEventGuestCount(subEvent.id)}</div>
      ...
      <div className={styles.addGuestSection}>
        ...
        <div className={styles.importMethods}>
          ...
          >
            ...
            <div className={styles.importDescription}>Upload a CSV or Excel file with your guest information</div>
          </div>
          <div
          ...
          >
            ...
            <div className={styles.importDescription}>Enter guest details and create groups automatically</div>
          </div>
        </div>
          ...
          <div className={styles.uploadSection}>
            <div className={styles.formGroup}>
              ...
              <div className={styles.fieldHelp}>
                Expected format: Name, Email, Phone, Gender, Tag, Group, [Sub-Event Names]<br />
                Leave Group empty for individual guests, or use same Group name for families
              </div>
            </div>
          </div>
        )}
        ...
        <div className={styles.guestFormOverlay}>
          <div className={styles.guestFormModal}>
            <div className={styles.modalHeader}>
              <h3 className={styles.modalTitle}>
                {editingGuest ? 'Edit Guest' :
                  addMode === 'group' ? 'Create Group & Add Members' :
                    'Add Individual Guest'}
              </h3>
                  ...
                  <div className={styles.formSectionGroup}>
                        ...
                        <div className={styles.formGroup}>
                          <label className={styles.checkboxOption}>
                            <input
                              ...
                              onChange={(e) => setNewGuest({ ...newGuest, isPointOfContact: e.target.checked })}
                            />
                    ...
                    {tempGroupMembers.length > 0 && (
                      <div className={styles.membersList}>
                        <h5 className={styles.membersListTitle}>Group Members ({tempGroupMembers.length})</h5>
                        {tempGroupMembers.map((member, index) => (
                          <div key={index} className={styles.memberCard}>
                            <div className={styles.memberInfo}>
                              <div className={styles.memberName}>
                                {member.name}
                                {member.isPointOfContact && <span className={styles.pocBadge}>POC</span>}
                              </div>
                              <div className={styles.memberDetails}>
                                {member.email} • {member.phone || 'No phone'} • {member.gender || 'No gender'}
                              </div>
                              {member.tag && <div className={styles.memberTag}>{member.tag}</div>}
                            </div>
                    ...
                    <div className={styles.formSectionGroup}>
                      <h4 className={styles.formSectionTitle}>Sub-Event Invitations (applies to all members)</h4>
                      <div className={styles.subEventGrid}>
                        {subEvents.map(subEvent => (
                          <label key={subEvent.id} className={styles.checkboxOption}>
                            <input
            ...
            <div className={styles.modalFooter}>
              ...
              >
                {editingGuest ? 'Save Changes' :
                  addMode === 'individual' ? 'Add Guest' :
      ...
      {totalGuests > 0 && (
        <div className={styles.guestListSection}>
          <div className={styles.subsectionHeader}>
            <h3 className={styles.subsectionTitle}>All Guests ({totalGuests})</h3>
          </div>
            ...
            <table className={styles.guestTable}>
              <thead>
                <tr>
                  ...
                  <th>Tag</th>
                  {subEvents.map(subEvent => (
                    <th key={subEvent.id} className={styles.subEventColumn}>{subEvent.name}</th>
                  ))}
                  <th>Actions</th>
                </tr>
              </thead>
              <tbody>
                {(eventData.guests || []).map(guest => {
                  ...
                  return (
                    <tr key={guest.id} className={isPointOfContact ? styles.pointOfContactRow : ''}>
                      <td>
                          ...
                          {group && (
                            <>
                              <div
                                ...
                                style={{ backgroundColor: group.details?.color || '#7c3aed' }}
                              />
                      ...
                      {subEvents.map(subEvent => (
                        <td key={subEvent.id} className={styles.eventCheckbox}>
                          <input
                            type="checkbox"
                            checked={guest.subEventRSVPs?.[subEvent.id] === 'invited'}
                            onChange={(e) => handleUpdateSubEventRSVP(guest.id, subEvent.id, e.target.checked)}
                          />
                        </td>
                      ))}
                      <td>
